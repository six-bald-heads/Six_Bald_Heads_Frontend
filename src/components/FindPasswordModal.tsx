...
const FindPasswordModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const handleSend = async () => {
    if (emailRegex.test(email)) {
      ...
      const url =
        "http://ec2-3-34-131-210.ap-northeast-2.compute.amazonaws.com:8080/api/v1/auth/findpassword";
      const data = {
        email,
      };
  ...
  return (
    <ModalWrapper>
      <ModalContent>
        ...
        <SendButton onClick={handleSend} isDisabled={!email} disabled={!email}>
          임시 비밀번호 전송
        </SendButton>
